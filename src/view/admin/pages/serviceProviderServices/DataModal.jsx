...
function DataModal({ show, setShow, data, modalType }) {
  ...
  const handleSave = (e) => {
    ...
    // ============================================
    // >>>>>>>>>>>>>>>>>>>>> update needed
    const addUrl = `${baseUrl}/admin/serviceproviderservice/add`;
    const editUrl = `${baseUrl}/admin/serviceproviderservice/update/${data.selectedId}`;
    form_data.append("icon", imgRef1.current.files[0]);
    form_data.append("name", data.name);
    form_data.append("description", data.description);
    form_data.append(
      "serviceProviderServiceCategory",
      data.serviceProviderServiceCategory
    );
