...
function DataModal({ show, setShow, data }) {
  ...
  const approveRequest = (newStatus) => {
    const url = `${baseUrl}/admin/doctor/update/${data.selectedId}`;
    const bodyData = new FormData();
    if (data.selectedStatus === "organizationUpdated") {
      bodyData.append("workingOrganizationsIsUpdated", newStatus);
      if (newStatus === "verified") {
        bodyData.append(
          "approvedWorkingOrganizations",
          JSON.stringify(data.workingOrganizations)
        );
      }
    } else {
        ...
        <Modal.Body>
          ...
          <Form>
            ...
            <Form.Group className="mb-3" controlId="formBasicEmail">
              <Form.Label>organizationOrIndivisual</Form.Label>
              <Form.Control
                type="text"
                value={getThisKeyFromThatKeyFromArray(
                  "_id",
                  data.organizationOrIndivisual,
                  "name",
                  data.allServiceProviderType
                )}
                onChange={() => {}}
                placeholder=""
              />
            ...
            <Modal.Footer>
              ...
              {data.selectedStatus === "organizationUpdated" && (
                <>
                  <Button
                    type="button"
                    variant="danger"
              ...
              {data.selectedStatus !== "organizationUpdated" && (
                <>
                  {(data.status === "verified" || !data.status) && (
                    <Button
                      type="button"
                      variant="danger"
                      onClick={() => {
                        approveRequest("blocked");
                      }}
                    >
                      Block Profile
                    </Button>
