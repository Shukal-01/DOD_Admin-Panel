...
// >>>>>>>>>>>>>>>>>>>>> update needed
const RenderValue = ({ givenValue, index, handleEdit, handleDelete, allDepartmentData }) => {
    ...
    return (<tr key={index}>
        ...
        <td>
            {/* {givenValue.departmentId} */}
            {allDepartmentData && allDepartmentData.length > 0 &&
                allDepartmentData.filter((value) => value.departmentId === givenValue.departmentId)[0]?.name}
        </td>
...
export const TableCmp = ({ data, handleEdit, handleDelete, allDepartmentData }) => {
                ...
                <tbody className="gRow gContent w-100">
                    {!(data && data.length) ? (<></>) : (
                        data.map((value, index) => {
                            return <RenderValue key={index} index={index} givenValue={value} handleEdit={handleEdit} handleDelete={handleDelete} allDepartmentData={allDepartmentData} />
                        })
                    )}
                </tbody>
            </table>
