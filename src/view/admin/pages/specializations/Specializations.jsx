...
import { getDepartmentList, getSpecializations } from "../../../../redux/action";
...
const Specializations = () => {
    ...
    // >>>>>>>>>>>>>>>>>>>>> update needed
    const allSpecialization = useSelector((state) => state.specializationReducer)
    // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< extra 
    ...
    const handleEdit = (givenValue, type) => {
        ...
        setShow(true);
        // >>>>>>>>>>>>>>>>>>>>> update needed
        setModalTitle(`${textCapitalize(type)} Doctor Specialization`);
        ...
        handleSetValue(givenValue.departmentId, setdepartmentId, allDepartments[0]?.departmentId)
        // ==========================
    }
    ...
    const handleDelete = (givenValue) => {
        ...
        if (confirm('are you sure you want to delete!')) {
            ...
            fetch(`${baseUrl}/admin/specialization/delete/${givenValue._id}`, {
                method: 'DELETE',
                headers: new Headers({
                ...
                .catch((error) => {
                    console.log('Specializations.jsx : handleDelete : 74');
                    console.log(error);
        ...
        <div className="gBox h-100" >
            <div className="gRow gHeader pb-3">
                <div className="gDflex gDjcsb gDaic">
                    <div>
                        <h5 className="tw_700">Specializations</h5>
                    </div>
                    <div>
            ...
            <div className="gRow gContent gDcol bodyContent" style={{ backgroundColor: bgColor.white }}>
                <div className="gBox">
                    <div className="gRow gContent w-100" style={{ overflow: 'scroll' }} >
                        <TableCmp data={allSpecialization} handleEdit={handleEdit} handleDelete={handleDelete} allDepartmentData={allDepartments} />
                    </div>
                </div>
            </div>
