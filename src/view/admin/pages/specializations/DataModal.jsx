...
function DataModal({ show, setShow, data, modalType }) {
    ...
    const handleSave = (e) => {
        ...
        // ============================================
        // >>>>>>>>>>>>>>>>>>>>> update needed
        const addUrl = `${baseUrl}/admin/specialization/add`;
        const editUrl = `${baseUrl}/admin/specialization/update/${data.selectedId}`
                ...
                <Modal.Body>
                    ...
                    <Form onSubmit={handleSave}>
                        ...
                        <Form.Group className="mb-3" controlId="formBasicEmail">
                            ...
                            <Form.Select value={data.departmentId}
                                onChange={(e) => {
                                    data.setdepartmentId(e.target.value)
                                }} name='status'>
                                {data.allDepartments && data.allDepartments.length > 0 &&
                                    data.allDepartments.map((value, index) => {
                                        return <option key={index} value={value.departmentId}>{value.name}</option>
                                    })}
                            </Form.Select>
