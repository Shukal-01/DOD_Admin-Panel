...
const RenderValue = ({ givenValue, index, handleEdit, handleDelete }) => {
    return (<tr key={index}>
        <th>{index + 1}.</th>
        <td><img  style={{ background: bgColor.imgTintColour }} src={givenValue.image} height={50} className="rounded" alt="" /></td>
        <td>{givenValue.name}</td>
        <td>{givenValue.email}</td>
        <td>{givenValue.employeeId}</td>
        <td>{givenValue.mobileNumber}</td>
        <td>{givenValue.designation}</td>
