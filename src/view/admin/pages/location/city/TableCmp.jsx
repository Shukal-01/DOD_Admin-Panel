...
// >>>>>>>>>>>>>>>>>>>>> update needed
const RenderValue = ({ givenValue, index, handleEdit, handleDelete, allCountries, allStates }) => {
    return (<tr key={index}>
        <th>{index + 1}.</th>
        ...
        <td>
            {allStates && allStates.length > 0 &&
                allStates.filter((value) => value._id === givenValue.stateCode)[0]?.name}
        </td>
        <td>
...
export const TableCmp = ({ data, handleEdit, handleDelete, allCountries, allStates }) => {
                ...
                <tbody className="gRow gContent w-100">
                    {!(data && data.length) ? (<></>) : (
                        data.map((value, index) => {
                            return <RenderValue key={index} index={index} givenValue={value} handleEdit={handleEdit} handleDelete={handleDelete} allCountries={allCountries} allStates={allStates} />
                        })
                    )}
                </tbody>
            </table>
